...
LRESULT CALLBACK VentanaRegistroUsuario(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND: {
		...
		case ID_BTN_RegUssOK: {
			...
			//nuevo->foto = hBitmapGlobal; C:\\Users\\isaia\\OneDrive\\Escritorio\\thor.JPG
			//Problema 1, la ruta parace estar demasiado grande, funciona hasta donde lo tenemos
			//si abro la imagen ya no crea el archivo.
			VETERINARIO* nuevo = crearUsuario(nombreVeterinarioA, cedulaA, usuarioA, contrasenaA, "C:\\Users\\isaia\\");
			if (nuevo == nullptr)
				break;
...
LRESULT CALLBACK VentanaPrincipal(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch (msg) {
	case WM_INITDIALOG: {
		HWND hwndTextBox1 = GetDlgItem(hWnd, ID_EC_LoginUss);
		SetWindowText(hwndTextBox1, L"isa");

		HWND hwndTextBox = GetDlgItem(hWnd, ID_EC_LoginPassword); //REMOVER
		SetWindowText(hwndTextBox, L"isa");
	}break;
	case WM_COMMAND: {
		int wmId = LOWORD(wParam);
		switch (wmId) {
...
LRESULT CALLBACK VentanaRegistrarCita(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG: {
		...
		HWND hwndTextBox = GetDlgItem(hWnd, ID_EC_RegCitaNombre);
		SetWindowText(hwndTextBox, L"Olga");

		HWND hwndTextBox1 = GetDlgItem(hWnd, ID_EC_RegCitaTel); //REMOVER
		SetWindowText(hwndTextBox1, L"3338211317");

		HWND hwndTextBox2 = GetDlgItem(hWnd, ID_EC_RegCitaMascota); //REMOVER
		SetWindowText(hwndTextBox2, L"Thor");

		HWND hwndTextBox3 = GetDlgItem(hWnd, ID_EC_RegCitaCosto); //REMOVER
		SetWindowText(hwndTextBox3, L"10");

		HWND hwndTextBox4 = GetDlgItem(hWnd, ID_EC_RegCitaMotivo); //REMOVER
		SetWindowText(hwndTextBox4, L"Vacunas");
...
LRESULT CALLBACK VentanaEliminarCita(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND: {
		...
		case ID_BTN_EliminarOK: {
			...
			NODOCITA* cita = buscarCita(nombreClienteA, "", cedulaUsuario);//REVISAR
			if (!esFechaValida(cita->cita->fechaCita)) {
			...
			string mascota = "";
			CITA* citaEliminada = EliminarCitaMedio(nombreClienteA, mascota, cedulaUsuario);//REVISAR
...
bool buscarFechaDisponible(const string& fecha, const string& hora, const string& cedulaUsuario) {
	...
	while (indice != NULL) {
		if (indice->cita->fechaCita == fecha && indice->cita->horaCita == fecha && indice->cita->cedulaVeterinario == cedulaUsuario)
			return false;
...
bool esFechaHoraValida(const string& fechaCita, const string& horaCita) {
	...
	SYSTEMTIME dateToCompare = { 0 };
	dateToCompare.wDay = dia;
	dateToCompare.wMonth = mes;
	dateToCompare.wYear = year;
	...
	if (dateToCompare.wYear < currentDate.wYear) {
		return false;
	}
	else if (dateToCompare.wYear == currentDate.wYear) { //REVISAR
		if (dateToCompare.wMonth < currentDate.wMonth) {
			return false;
		}
		else if (dateToCompare.wMonth == currentDate.wMonth) {
			if (dateToCompare.wDay < currentDate.wDay)
				return false;
		}
	}
	...
	if ((dateToCompare.wYear == currentDate.wYear) && (dateToCompare.wMonth == currentDate.wMonth) && (dateToCompare.wDay == currentDate.wDay)) {
...
bool citaEsParaHoy(const string& fechaCita)
{
	...
	// Configurar la estructura SYSTEMTIME con la fecha parseada
	SYSTEMTIME dateToCompare = { 0 };
	dateToCompare.wDay = day;
	dateToCompare.wMonth = month;
	dateToCompare.wYear = year;
...
bool esFechaValida(const string& fechaCita)
{
	...
	else if (dateToCompare.wYear == currentDate.wYear)
	{
		if (dateToCompare.wMonth < currentDate.wMonth)
		{
			return false;
		...
		{
			return dateToCompare.wDay >= currentDate.wDay;
		}
	}
